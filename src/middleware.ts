...
// export function middleware(request: NextRequest) {
//   // Sekarang kita mencari cookie dengan nama yang benar dari FastAPI: 'access_token'
//   const isLoggedIn = request.cookies.has('access_token'); 
...
//   const isPublicPage = path === '/login' || path === '/register' || path === '/forgot-password' || path === '/verify';

//   // 1. Jika BELUM LOGIN dan mencoba akses halaman utama (/), lempar ke /login
//   if (!isLoggedIn && !isPublicPage) {
//     return NextResponse.redirect(new URL('/login', request.url));
//   }

//   // 2. Jika SUDAH LOGIN tapi mencoba buka halaman public (/login, /register), lempar ke halaman utama (/)
//   if (isLoggedIn && isPublicPage) {
//     return NextResponse.redirect(new URL('/', request.url));
//   }
...
// // Konfigurasi ini mengatur halaman mana saja yang akan diawasi oleh middleware
// export const config = {
//   matcher: [
//     /*
//      * Awasi semua rute KECUALI:
